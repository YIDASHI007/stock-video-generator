...
推送 `v0.1.5` 形式的标签后，`.github/workflows/release.yml` 会运行测试、构建安装包，并使用 `RELEASE_REPO_TOKEN` 在公开的 `YIDASHI007/stock-video-generator-releases` 仓库创建 GitHub Release。应用启动后会读取 `resources/update.json`，检查该仓库中的新版本；用户确认后下载、关闭本机后台服务、安装并重启。
